...
namespace Common
{
    void RegularMesh::flood(int start, const AABB& aabb)
    {
        int current = start;
        Bin& bin = bin_p(start);
        bin.set_load(-1);
    ...
    void RegularMesh::move(const vec3<double>& v)
    {
        aabb_.move_points(v);
        for (Bin& b: bin_)
        {
    ...
    void RegularMesh::rotate(double ang, int axis, const vec3<double>& rot_point)
    {
    ...
    int get_binindex(unsigned int index, RelativePosition rp, const vec3<unsigned int>& nstripe, unsigned int binsize)
    {
        ...
        switch (rp)
        {
            case RelativePosition::cc:
                return index;
            case RelativePosition::cw:
                if (index_have_west())
                    return index - 1;
            case RelativePosition::ce:
                if (index_have_east())
                    return index + 1;
            case RelativePosition::sc:
                if (index_have_south())
    ...
    std::vector<int> RegularMesh::pneis_of_bin(unsigned int index)
    {
        assert(index >= 0);
        ...
        if (index_have_east())
            nei[1] = index + 1;
        if (index_have_south())
            nei[2] = index - nstripe_(0);
        if (index_have_north())
            nei[3] = index + nstripe_(0);
    ...
    int RegularMesh::get_binindex(unsigned int index, RelativePosition rp)
    {
        ...
        switch (rp)
        {
            ...
            case RelativePosition::sc:
                if (index_have_south())
                    return index - nstripe_(0);
            case RelativePosition::sw:
                if (index_have_south() && index_have_west())
                    return index - nstripe_(0) - 1;
            case RelativePosition::se:
                if (index_have_south() && index_have_east())
                    return index - nstripe_(0) + 1;
            case RelativePosition::nc:
    ...
    {
        //assert(tag_() == 0);
        update_address(rmtag_address_map_);
    }

    void RegularMesh::update_address(std::map<Tag, RegularMesh*>& map)
    {
        if (map.empty()) {
            return;
        }
    ...
    bool RegularMesh::is_resident(const vec3<double>& cnt) const
    {
    ...
    //void RegularMesh::set_aabb_min(const vec3<double>& m)
    //{
        //aabb_.set_min(m);
    //}
    //void RegularMesh::set_aabb_max(const vec3<double>& m)
    //{
        //aabb_.set_max(m);
    //}
    ...
    void RegularMesh::set_nstripe(const vec3<int>& ns)
    {
        set_nstripe(ns(0), ns(1), ns(2));
    }

    void RegularMesh::set_h(const vec3<double>& v)
    {
        set_h(v(0), v(1), v(2));
    }
    ...
    const vec3<double>& RegularMesh::h() const
    {
        return h_;
    }

    const vec3<int>& RegularMesh::nstripe() const
    {
        return nstripe_;
    }
    ...
    RelativePosition get_relative_position(int index, int targetindex, const vec3<unsigned int>& nstripe, size_t maxbinsize, bool& exact)
    {
        ...
        if (index == targetindex) {return RelativePosition::cc;}
        if (index == targetindex+1 && index_have_west()) {return RelativePosition::cw;}
        if (index == targetindex-1 && index_have_east()) {return RelativePosition::ce;}
        if (index == targetindex-nstripe(0) && index_have_north()) {return RelativePosition::nc;}
        if (index == targetindex-nstripe(0)+1 && index_have_west() && index_have_north()) {return RelativePosition::nw;}
        if (index == targetindex-nstripe(0)-1 && index_have_east() && index_have_north()) {return RelativePosition::ne;}
        if (index == targetindex+nstripe(0) && index_have_south()) {return RelativePosition::sc;}
        if (index == targetindex+nstripe(0)+1 && index_have_west() && index_have_south()) {return RelativePosition::sw;}
        if (index == targetindex+nstripe(0)-1 && index_have_east() && index_have_south()) {return RelativePosition::se;}
    ...
    vec3<double> RegularMesh::centroid(const RegularMeshIndex& index) const
    {
        assert(index.i() < 0);
        assert(index.j() < 0);
    ...
    vec3<double> llcoor(const RegularMeshIndex& index, const vec3<double>& aabb_min, const vec3<double>& h)
    {
        ...
        return vec3<double>(d0, d1, d2);
    }

    vec3<double> RegularMesh::llcoor(const RegularMeshIndex& index) const
    {
    ...
    void RegularMesh::insert_bins(int mintag, int mesh_load_size)
    {
        bin_.clear();
        ...
        for (int k=0; k<nstripe(2); ++k)
        {
            for (int i=0; i<nstripe(1); ++i)
            {
                ...
                {
                    _index.set(i, j, k);
                    vec3<double> ll = llcoor(_index);
                    vec3<double> ur = ll + h();

                    assert(tag_.isvalid());

                    Bin temp_bin(current_tag, tag_, _index, AABB(ll, ur), mesh_load_size);
        ...
        {
            assert(b.cell().empty());
            assert(b.mesh_tag_index_map().empty());
    ...
    void RegularMesh::calc_aabb_max()
    {
        ...
        vec3<double> max_aabb(max_0, max_1, max_2);
    ...
    bool RegularMesh::get_index_regular(const vec3<double>& cv, std::vector<RegularMeshIndex>& index, bool closest) const
    {
        if (closest)
        {
            vec3<double> newcv(cv);
            ...
            return get_index_regular(newcv, index, false);
            //return;
        }
        else
        {
        ...
        if (ZERO < aabb_.min(0) - cv(0)) return false;
        if (ZERO < aabb_.min(1) - cv(1)) return false;
        if (ZERO < aabb_.min(2) - cv(2)) return false;
        if (cv(0) - aabb_.max(0) > ZERO) return false;
        if (cv(1) - aabb_.max(1) > ZERO) return false;
        if (cv(2) - aabb_.max(2) > ZERO) return false;
        ...
        if (std::abs(_d0 - std::round(_d0)) < ZERO)
        {
            _d0 = std::round(_d0);
            remx_is_zero = true;
        }
        if (std::abs(_d1 - std::round(_d1)) < ZERO)
        {
            _d1 = std::round(_d1);
            remy_is_zero = true;
        }
        if (std::abs(_d2 - std::round(_d2)) < ZERO)
        {
            _d2 = std::round(_d2);
            remz_is_zero = true;
        }
    ...
    bool RegularMesh::get_bintag_adaptive(const vec3<double>& cv, std::vector<BinRMTag>& tag, bool closest) const
    {
    ...
    /*bool RegularMesh::get_bintag_regular(const vec3<double>& cv, std::vector<BinRMTag>& tag, bool closest) const
    {
        std::vector<RegularMeshIndex> _index;
        bool res = get_index_regular(cv, _index, closest);
        if (res == false)
    ...
    /*bool RegularMesh::get_index_adaptive(const vec3<double>& cv, std::vector<RegularMeshIndex>& index, bool closest) const
    {
        std::vector<RegularMeshIndex> _index;
        bool res = get_index_regular(cv, _index, closest);
        if (res == false)
    ...
    void RegularMesh::get_bintag_adaptive(const AABB& aabb, std::vector<BinRMTag>& tag, int& nlevel) const
    {
        nlevel = 0;
        get_bintag_adaptive_(aabb, tag, nlevel);
        std::sort(tag.begin(), tag.end(), &BinRMTag::sorter);
        tag.erase(std::unique(tag.begin(), tag.end()), tag.end());
    }

    void RegularMesh::get_bintag_adaptive_(const AABB& aabb, std::vector<BinRMTag>& tag, int& nlevel) const
    {
        ++nlevel;
        std::vector<RegularMeshIndex> _index;
        //_index.reserve(64);
        ...
        p.reserve(8);
        p.push_back(vec3<double>(aabb.min(0), aabb.min(1), aabb.min(2))); // xmin, ymin, zmin
        p.push_back(vec3<double>(aabb.min(0), aabb.max(1), aabb.min(2))); // xmin, ymax, zmin
        p.push_back(vec3<double>(aabb.max(0), aabb.min(1), aabb.min(2))); // xmax, ymin, zmin
        p.push_back(vec3<double>(aabb.max(0), aabb.max(1), aabb.min(2))); // xmax, ymax, zmin

        p.push_back(vec3<double>(aabb.min(0), aabb.min(1), aabb.max(2))); // xmin, ymin, zmax
        p.push_back(vec3<double>(aabb.min(0), aabb.max(1), aabb.max(2))); // xmin, ymax, zmax
        p.push_back(vec3<double>(aabb.max(0), aabb.min(1), aabb.max(2))); // xmax, ymin, zmax
        p.push_back(vec3<double>(aabb.max(0), aabb.max(1), aabb.max(2))); // xmax, ymax, zmax

        for (const auto& _p: p) 
        {
            std::vector<RegularMeshIndex> __index;
            //__index.reserve(8);
            //get_index_regular(_p, __index, true); // why closest=true?
            get_index_regular(_p, __index, false); // why closest=true?
        ...
        for (const auto& i: _index)
        {
            ...
            subrm->get_bintag_adaptive_(aabb, tag, nlevel);
        }
    ...
    /*bool RegularMesh::get_bintag_regular(const AABB& aabb, std::vector<Tag>& tag) const
    {
        std::vector<RegularMeshIndex> _index;
        std::vector<vec3<double>> p;
        p.push_back(vec3<double>(aabb.min(0), aabb.min(1))); // xmin, ymin
        p.push_back(vec3<double>(aabb.min(0), aabb.max(1))); // xmin, ymax
        p.push_back(vec3<double>(aabb.max(0), aabb.min(1))); // xmax, ymin
        p.push_back(vec3<double>(aabb.max(0), aabb.max(1))); // xmax, ymax
    ...
    bool RegularMesh::point_inside_bin(const RegularMeshIndex& ind, const vec3<double>& p)
    {
        vec3<double> ll = llcoor(ind);
    ...
    //void RegularMesh::print(std::string file_name, const std::deque<Mesh>& mesh) const
    void RegularMesh::print(std::string file_name) const
    {    
        std::ofstream out;    
        out.open (file_name);
        ...
        out << "DIMENSIONS " << nstripe(0)+1 << " " << nstripe(1)+1 << " " << 1 << std::endl;
        out << "POINTS " << (nstripe(0)+1)*(nstripe(1)+1) << " float" <<  std::endl;
        ...
        out << "CELL_DATA " << nstripe(0)*nstripe(1) << std::endl;
    ...
    void RegularMesh::print(std::string file_name, const std::map<BinRMTag, int>& bintag_proc_map) const
    {    
        std::ofstream out;    
        out.open (file_name);
        ...
        for (int k=0; k<nstripe(2); ++k)
        {
            for (int i=0; i<nstripe(1); ++i)
            {
                for (int j=0; j<nstripe(0); ++j)
                {
                    const auto& b = bin(i,j,k);
                    if (b.rm() == nullptr)
                    {
                        //bool found = false;
                        for (const auto& p: bintag_proc_map)
                        {
                            if (p.first.rmtag() == tag_ && p.first.bintag() == b.tag())
                            {
                                //out << b.load(mesh);
