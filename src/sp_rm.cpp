...
namespace Common
{
    void SpatialPartition::make_regular_map_for_mesh_serial(const Tag& mt)
    {
        //if (!master())
        {
            ...
            rm_p(mt).insert_bins(0, 0);
            assert(!mesh(mt).cell().empty());
            //assert(!rm_p(mt).bintag_index_map().left.empty());
            //std::cout << "registering mesh to map" << std::endl;
            rm_p(mt).register_mesh(mesh(mt), false, world_.rank(), false, nullptr);
        }
    }
    
    void SpatialPartition::make_regular_map_for_mesh_uniproc(const Tag& mt, const std::shared_ptr<Profiler>& profiler_)
    {
        //AABB mesh_aabb(mesh(mt).rawpoint());
        ...
        rm_p(mt).register_mesh(mesh(mt), false, world_.rank(), false, profiler_);
    ...
    void SpatialPartition::make_regular_map_for_mesh(const Tag& mt)
    {
        if (!master())
        {
            ...
            rm.register_mesh(msh, false, world_.rank(), false, nullptr);
    ...
    void SpatialPartition::make_regular_maps_for_mesh_uniproc(const std::shared_ptr<Profiler>& profiler_)
    {
